...
Instead of using the textbook algorithm, a more recent paper algorithmic framework is adapted to the problem, shown in Algorithm \ref{}

\begin{algorithm}[H]
\SetAlgoLined
\KwIn{penalty parameter $\beta$, stopping tolerance $\tau$}
weight and state variables $x_0$,  $\lambda_0 = \mathcal{N}(0,1)$ \;
 \For{k = 0,1,...}{
 	$\eta_k = 1/\beta^k$\;
 	find $x_{k+1}$ such that \\
 	$||\nabla_xM_{\beta}(x,\lambda_k)|| \leq \eta_k$ \\
 	$\sigma_{k+1} = \text{min}\big(\frac{||h(x_0)||\log^22}{||h(x_k+1)||k \log^2(k+1)},1\big)$ \\
